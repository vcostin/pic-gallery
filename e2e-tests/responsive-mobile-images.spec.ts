...
test.describe('Responsive & Mobile Image Gallery - E2E Tests', () => {
  ...
  test('should display mobile layout correctly on small screens', async ({ page }) => {
    ...
    // Try to login first - if it fails, create a new user
    let loginSuccess = await TestHelpers.quickLogin(page, TEST_USER.email, TEST_USER.password);
    
    if (!loginSuccess) {
      console.log('🔄 User not found, creating new test user...');
      const registerSuccess = await TestHelpers.quickRegister(page, TEST_USER.email, TEST_USER.password);
      expect(registerSuccess).toBe(true);
      loginSuccess = await TestHelpers.quickLogin(page, TEST_USER.email, TEST_USER.password);
    }
